...
int main(int argc, char* argv[]) {
    ...
    analyzer.update_full(game);
    int previous_drop_off_count = 1;
    ...
    for (;;) {
        ...
        int current_dropoff_count = 1 + static_cast<int>(me->dropoffs.size());
        bool new_dropoff_created = (current_dropoff_count > previous_drop_off_count);

        if (new_dropoff_created) {
            LOG("New dropoff created! Total" + to_string(current_dropoff_count))
                previous_drop_off_count = current_dropoff_count;
        }

        analyzer.update(game, false, new_dropoff_created);
